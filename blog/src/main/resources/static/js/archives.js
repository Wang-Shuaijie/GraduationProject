    ...
    function putInArchivesArticleInfo(data){
        ...
        timeline.append('<div class="timeline-row">' +
            ...
            '</span>' +
            '<h1 class="title  am-animation-slide-top"># <span class="archivesNum">根据时间查找</span></h1>' +
            '</div>');
        var strArray = new Array();
        $.each(data['result'], function (index, obj) {
            var year = obj['rptTime'].substring(0, 4);
            var month = obj['rptTime'].substring(5, 7);
            ...
            content.append($('<header class="am-comment-hd" style="background: #fff">' +
                ...
                '<a href="/article/' + obj['id'] + '">' + obj['title'] + '</a>' +
                '</div>' +
                '</header>'));
            var amCommentBd = $('<div class="am-comment-bd"></div>');
            amCommentBd.append($('<i class="am-icon-calendar"> <a href="/archives?archive=' + obj['rptTime'] + '">' + timestampToYMDTime(obj['rptTime']) + '</a></i>' +
